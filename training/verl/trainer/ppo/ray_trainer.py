...
from verl.utils.metric import MetricFunc
from verl.utils.respense_check import detect_language_mix, check_repetition
...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        self.metric_func = MetricFunc(self.tokenizer)
        ...
        adapt_ent_config = self.config.actor_rollout_ref.actor.adaptive_entropy
        use_adapt_ent = adapt_ent_config.enabled
        self.ent_ctrl = core_algos.EntController(init_ent_coef=self.config.actor_rollout_ref.actor.entropy_coeff,
                                                 ...
                                                 target_ent=adapt_ent_config.target_entropy,
    ...
    def _validate_with_temperatures(self, temperatures=[0.6, 1.0]):
        """Validate model with multiple temperatures.
        
        Args:
            temperatures: List of temperatures to validate with
            
        Returns:
            Dictionary containing validation metrics for all temperatures
        """
        # Save original temperature
        original_val_temperature = self.config.actor_rollout_ref.rollout.val_temperature
        
        val_metrics_suffixed = {}
        for temp in temperatures:
            # Set the temperature for validation
            self.config.actor_rollout_ref.rollout.val_temperature = temp
            val_metrics = self._validate()
            # Suffix metric keys with the temperature
            val_metrics_suffixed.update({f'{k}_t{temp}': v for k, v in val_metrics.items()})
        
        # Restore original temperature
        self.config.actor_rollout_ref.rollout.val_temperature = original_val_temperature
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_batch = test_batch.repeat(repeat_times=n_val_samples, interleave=False)
            ...
            extra_info.extend(test_batch.non_tensor_batch['extra_info'].tolist())
        ...
        for i in range(reward_tensor.shape[0]):
            ...
            source_name_list = ['/'.join(data_source.split('-')[:i+1]) for i in range(len(data_source.split('-')))]
            for source_name in source_name_list:
                data_source_reward[source_name].append(reward_tensor[i].item())

        metric_dict = {}
        for data_source, rewards in data_source_reward.items():
            metric_dict[f'val/{data_source}'] = np.mean(rewards)
        metric_dict = dict(sorted(metric_dict.items()))

        save_val_stats_path = getattr(self.config.trainer, 'stats_path', None)
        if save_val_stats_path is not None:
            os.makedirs(save_val_stats_path, exist_ok=True)
            val_stats_file = os.path.join(save_val_stats_path, f"val_stats_step_{self.global_steps}_temperature_{self.config.actor_rollout_ref.rollout.val_temperature}.jsonl")
            # Gather prompts and responses
            prompts = sample_inputs
            responses = sample_outputs
            prompt_lengths = [
                len(self.tokenizer.encode(prompt, add_special_tokens=False)) for prompt in sample_inputs
            ]
            response_lengths = [
                len(self.tokenizer.encode(response, add_special_tokens=False)) for response in sample_outputs
            ]                
            with open(val_stats_file, "w") as f:
                for i in range(len(prompts)):
                    data_to_save = {
                        ...
                        "prompt_length": self._convert_numpy_types(prompt_lengths[i]),
                        "response_length": self._convert_numpy_types(response_lengths[i]),
                        "reward": self._convert_numpy_types(sample_scores[i]),
                        "data_source": self._convert_numpy_types(data_sources[i]),
                        "ability": self._convert_numpy_types(abilities[i]),
            ...
            val_metrics_file = os.path.join(save_val_stats_path, f"val_metrics_steps_all_temperature_{self.config.actor_rollout_ref.rollout.val_temperature}.jsonl")
            with open(val_metrics_file, "a") as f:
                metrics_to_save = self._convert_numpy_types({"global_steps": self.global_steps, **metric_dict})
    ...
    def _load_checkpoint(self):
        ...
        # load dataloader, # do not load old dataloader
    ...
    def _convert_numpy_types(self, obj):
        """Convert numpy types and datetime to native Python types for JSON serialization"""
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # 是否使用在线passrate过滤
                if self.config.algorithm.use_online_passrate_filter:
                    print(f"DEBUG: {batch_dict.keys()=}")
                    for k, v in batch_dict.items():
                        print(k, len(v), v[0])
                    
                    print(f"DEBUG: before", len(batch_dict["raw_prompt"]))
                    for query in batch_dict["raw_prompt"]:
                        if query not in query2passrate:
                            query2passrate[query] = [0] # 初始化
                    indices = [i for i, query in enumerate(batch_dict["raw_prompt"]) if query2passrate[query][-1] < 1.0]
                    batch_dict = {key: [value[i] for i in indices] for key, value in batch_dict.items()}
                    print(f"DEBUG: after", len(batch_dict["raw_prompt"]))
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        ...
                        # print(f'DEBUG: {len(gen_batch_output)=}') # 256
                    ...
                    # print(f"DEBUG: after {len(batch.batch)=}") # 256
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        # print(f"DEBUG: {len(reward_tensor)=}") # 256
                        batch.batch['token_level_scores'] = reward_tensor

                        # print(f"DEBUG: {batch.batch.keys()=}")
                        # ['input_ids', 'prompts', 'attention_mask', 'responses', 'position_ids', 'token_level_scores']
                        # print(f"DEBUG: {batch.non_tensor_batch.keys()=}")
                        ...
                        if self.config.algorithm.adv_estimator == 'grpo':
                            ...
                            # print(f"DEBUG: {len(uids)=}") # 256
                            unique_uids = np.unique(uids)
                            # print(f"DEBUG: {len(unique_uids)=}") # 64
                            ...
                            num_reward_clip = 0
                            num_language_mix = 0
                            num_response_repeat = 0

                            acc_dist = [0 for i in range(10)]
                            for uid in unique_uids:
                                uid_mask = uids == uid
                                # print(f"DEBUG: {len(uid_mask)=}") # 256
                                uid_rewards = reward_tensor[uid_mask].sum(-1)  # Sum rewards for each sequence
                                # print(f"DEBUG: {len(uid_rewards)=}") # 4

                                acc = uid_rewards.mean()
                                if self.config.algorithm.use_online_passrate_filter:
                                    query = data.batch["raw_prompt"][uid_mask][0]
                                    assert query in query2passrate, f"{query} not in query2passrate"
                                    query2passrate[query].append(acc)
                                ...
                                # 是否根据reward对query进行过滤
                                if self.config.algorithm.use_reward_clip:
                                    assert 0.0 <= acc <= 1.0, f'{acc=}, make sure the mean reward in [0, 1]'
                                    if not(self.config.algorithm.reward_clip_low < acc < self.config.algorithm.reward_clip_high):
                                        valid_mask[uid_mask] = False
                                        num_reward_clip += 1
                                        continue
                                
                                # if self.config.algorithm.use_response_filter:
                                prompts = self.tokenizer.batch_decode(batch.batch['prompts'][uid_mask], skip_special_tokens=True)
                                responses = self.tokenizer.batch_decode(batch.batch['responses'][uid_mask], skip_special_tokens=True)
                                uid_mask_ = []
                                for prompt, response in zip(prompts, responses):
                                    if detect_language_mix(prompt, response):
                                        num_language_mix += 1
                                        # print(f'detect_language_mix: {prompt=}\n{response=}')
                                        uid_mask_.append(True)
                                    elif check_repetition(response):
                                        num_response_repeat += 1
                                        # print(f'check_repetition: {response=}')
                                        uid_mask_.append(True)
                                    else:
                                        uid_mask_.append(False)
                                # 是否对生成的response进行检查，过滤掉重复、语言混杂等生成
                                if self.config.algorithm.use_response_filter:
                                    assert len(uid_mask_) == sum(uid_mask)
                                    j = 0
                                    for i in range(len(uid_mask)):
                                        if uid_mask[i]:
                                            uid_mask[i] = uid_mask_[j]
                                            j += 1
                                    assert sum(uid_mask_) == sum(uid_mask)
                                    valid_mask[uid_mask] = False

                            
                            # DEBUG: Log to metrics
                            # if self.config.algorithm.use_reward_clip:
                            metrics['DEBUG/num_reward_clip'] = num_reward_clip
                            metrics['DEBUG/num_reward_clip_ratio'] = num_reward_clip / len(unique_uids)
                            # if self.config.algorithm.use_response_filter:
                            metrics['DEBUG/num_language_mix'] = num_language_mix
                            metrics['DEBUG/num_language_mix_ratio'] = num_language_mix / len(batch.batch["responses"])
                            metrics['DEBUG/num_response_repeat'] = num_response_repeat
                            metrics['DEBUG/num_response_repeat_ratio'] = num_response_repeat / len(batch.batch["responses"])
                ...
                # At the end of the batch update logic:
                save_stats_path = getattr(self.config.trainer, 'stats_path', None)
                stats_save_freq = getattr(self.config.trainer, 'stats_save_freq', 1)
                current_global_steps = self.global_steps
                save_now = current_global_steps % stats_save_freq == 0

                # Save metrics for all steps
                if save_stats_path is not None:
                    with open(os.path.join(save_stats_path, "train_metrics_steps_all.jsonl"), "a") as f:
                        metrics_to_save = self._convert_numpy_types({"global_steps": self.global_steps, **metrics})
                        f.write(json.dumps(metrics_to_save, ensure_ascii=False) + "\n")
                ...
                rewards = tensor_batch["token_level_scores"].sum(-1).tolist()

                non_tensor_batch = batch.non_tensor_batch
                data_sources = non_tensor_batch["data_source"].tolist()
                abilities = non_tensor_batch["ability"].tolist()
                reward_models = []
                for rm in non_tensor_batch["reward_model"].tolist():
                    rm["ground_truth"] = rm["ground_truth"].tolist() if isinstance(rm["ground_truth"], np.ndarray) else rm["ground_truth"]
                    reward_models.append(rm)
                extra_info = non_tensor_batch["extra_info"].tolist()

                # Always append current batch stats to the accumulator
                for i in range(len(prompts)):
                    save_data.append({
                        "global_steps": self.global_steps,
                        "prompt": prompts[i],
                        ...
                        "prompt_length": self._convert_numpy_types(prompt_lengths[i]),
                        "response_length": self._convert_numpy_types(response_lengths[i]),
                        "reward": self._convert_numpy_types(rewards[i]),
                        "data_source": self._convert_numpy_types(data_sources[i]),
                        "ability": self._convert_numpy_types(abilities[i]),
                        "reward_model": self._convert_numpy_types(reward_models[i]),
                        "extra_info": self._convert_numpy_types(extra_info[i])
                ...
                # If it is time to save, write out the accumulated stats
                if save_stats_path is not None and save_now:
                    ...
                    save_data = []
                    current_interval_start = self.global_steps + 1
