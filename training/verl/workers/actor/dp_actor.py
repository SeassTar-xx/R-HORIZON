...
class DataParallelPPOActor(BasePPOActor):
    ...
    def get_entropy_metric(self,batch, ent_ctrl_dict):
        ...
        metrics = {
            ...
            "actor/pg_grad_ratio" : pg_grad_scale / (pg_grad_scale + entropy_grad_scale_with_coeff + 1e-8),
            "actor/entropy_grad_ratio" : entropy_grad_scale_with_coeff / (pg_grad_scale + entropy_grad_scale_with_coeff + 1e-8),
    ...
    def update_policy(self, data: DataProto):
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            for data in micro_batches:
                ...
                clip_ratio_high = self.config.clip_ratio_high # add by wjn
                entropy_coeff = ent_ctrl_dict["value"]
                entropy_loss_enabled = ent_ctrl_dict["entropy_loss_enabled"]
                ...
                metrics_dict = {
                    # loss
                    'actor/total_loss': policy_loss.detach().item(),

                    # policy loss
                    'actor/pg_loss': pg_loss.detach().item(),
                    'actor/pg_clipfrac': pg_clipfrac.detach().item(),
                    'actor/ppo_kl': ppo_kl.detach().item(),
                    'actor/pg_ratio': (pg_loss / policy_loss).detach().item(),

                    # entropy   
                    # 'actor/entropy_loss': entropy_loss.detach().item(),
                    # 'actor/entropy_coeff': entropy_coeff,
                    # 'actor/entropy_loss_with_coeff': entropy_term.detach().item(),
                    'actor/entropy_ratio': (entropy_term / policy_loss).detach().item(),
                    'actor/entropy_gt_pg': (entropy_term > pg_loss).float().detach().item(),
                }

                # Only add KL-related metrics if use_kl_loss is True
                if self.config.use_kl_loss:
                    kl_metrics = {
                        'actor/kl_loss': kl_loss.detach().item(),
                        'actor/kl_coef': self.config.kl_loss_coef,
                        'actor/kl_loss_with_coef': kl_term.detach().item(),
                        'actor/kl_ratio': (kl_term / policy_loss).detach().item(),
                        'actor/kl_gt_pg': (kl_term > pg_loss).float().detach().item(),
                    }
                    metrics_dict.update(kl_metrics)

                # Update the metrics dictionary with single values instead of trying to append
